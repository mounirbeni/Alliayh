    ...
    <div className="flex min-h-[calc(100vh-5rem)] items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="font-headline text-4xl">{pt ? "Criar conta" : "Create account"}</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            {pt ? "Junte-se à comunidade Lueur" : "Join the Lueur community"}
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-primary/10 bg-white/80 p-8 shadow-sm backdrop-blur">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{pt ? "Nome completo" : "Full name"}</FormLabel>
                    <FormControl>
                      <Input placeholder={pt ? "O seu nome" : "Your name"} className="rounded-xl" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>{pt ? "Confirmar palavra-passe" : "Confirm password"}</FormLabel>
                    <FormControl>
              ...
              <Button type="submit" disabled={isPending} className="w-full rounded-full" size="lg">
                {isPending ? (
                  <>
                    ...
                    {pt ? "A criar conta…" : "Creating account…"}
                  </>
                ) : (
          ...
          <div className="mt-6 text-center text-sm text-muted-foreground">
            ...
            <Link href={localizedPath("/auth/login", locale)} className="font-medium text-primary hover:underline">
              {pt ? "Entrar" : "Sign in"}
            </Link>
          </div>
        </div>
        ...
        <p className="mt-6 text-center text-xs text-muted-foreground">
          {pt ? "Ao registar-se, aceita os nossos" : "By registering, you agree to our"}{" "}
