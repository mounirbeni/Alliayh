        ...
        <div className="container mx-auto px-4 py-12">
                  ...
                  <TabsList className="bg-white dark:bg-black/20 border border-primary/10 rounded-full h-14 px-2 shadow-sm">
                    {categories.map(cat => (
                      <TabsTrigger key={cat} value={cat} className="rounded-full px-4 sm:px-6 h-10 data-[state=active]:bg-primary data-[state=active]:text-white font-headline uppercase tracking-widest text-[10px] sm:text-xs">
                        {cat === 'All' ? t.productsPage.all : cat}
                      </TabsTrigger>
                    ))}
                  </TabsList>
                </Tabs>
             ...
             <div className="flex overflow-x-auto w-full snap-x pb-2 lg:pb-0 lg:flex-wrap items-center gap-4 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
               ...
               <Popover>
                 <PopoverTrigger asChild>
                   <Button variant="outline" className="h-12 rounded-full border-primary/20 hover:border-primary px-6 flex gap-2 uppercase tracking-widest text-xs font-bold">
                     <Filter className="h-4 w-4" /> {t.productsPage.filters}
                 ...
                 <PopoverContent className="w-80 p-6 rounded-[2rem] border-primary/10 shadow-xl" align="start">
                   <div className="space-y-6">
                     <h4 className="font-headline uppercase tracking-widest text-sm border-b border-primary/10 pb-2">{t.productsPage.filterOptions}</h4>
                     
                     {/* Price Range */}
                     <div className="space-y-4">
                       <h5 className="text-xs uppercase tracking-widest text-muted-foreground font-bold">{t.productsPage.priceRange}</h5>
                       <div className="flex items-center gap-4">
                         <div className="relative flex-1">
                           <span className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground text-xs">{t.common.currency}</span>
                           <Input 
                             type="number" 
                     ...
                     {/* Key Ingredients */}
                     <div className="space-y-4">
                       <h5 className="text-xs uppercase tracking-widest text-muted-foreground font-bold">{t.productsPage.keyIngredients}</h5>
               ...
               {(minPrice !== '' || maxPrice !== '') && (
                 <Badge variant="secondary" className="h-10 px-4 rounded-full flex gap-2 items-center text-xs uppercase tracking-widest font-normal bg-primary/5 hover:bg-primary/10 border-none cursor-pointer" onClick={() => {setMinPrice(''); setMaxPrice('');}}>
                   {formatCurrency(Number(minPrice) || 0, locale)} - {maxPrice ? formatCurrency(Number(maxPrice), locale) : t.common.any} <X className="h-3 w-3 text-muted-foreground" />
                 </Badge>
               )}
             </div>

             <div className="flex items-center gap-2">
                <span className="text-xs uppercase tracking-widest font-headline text-muted-foreground hidden sm:inline-block">{t.productsPage.sortBy}</span>
                <Select value={sortOption} onValueChange={setSortOption}>
                  ...
                  <SelectContent className="rounded-2xl">
                    <SelectItem value="featured" className="text-xs uppercase tracking-widest p-3">{t.productsPage.featured}</SelectItem>
                    <SelectItem value="price-low" className="text-xs uppercase tracking-widest p-3">{t.productsPage.priceLowToHigh}</SelectItem>
                    <SelectItem value="price-high" className="text-xs uppercase tracking-widest p-3">{t.productsPage.priceHighToLow}</SelectItem>
                    <SelectItem value="rating" className="text-xs uppercase tracking-widest p-3">{t.productsPage.topRated}</SelectItem>
                  </SelectContent>
                </Select>
             </div>
          </div>
          ...
          <div className="mb-8 flex justify-between items-center px-2">
            <p className="text-sm text-muted-foreground italic">
              {t.productsPage.showing} {filteredProducts.length} {t.productsPage.results}
            </p>
            {categoryFilter !== 'All' && (
              <Button 
                variant="ghost" 
          ...
          {isLoading ? (
            ...
            <div className="py-32 text-center space-y-4">
              ...
              <h3 className="font-headline text-2xl">{t.productsPage.noProducts}</h3>
              <p className="text-muted-foreground italic max-w-md mx-auto">{t.productsPage.noProductsDesc}</p>
